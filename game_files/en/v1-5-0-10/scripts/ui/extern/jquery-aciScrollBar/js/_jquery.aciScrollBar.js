...
(function($){
    ...
    $.fn._aciScrollBar = function(options, data){
        ...
        var _customUi = function(){
            if ($this.data('customUi' + $.aciScrollBar.nameSpace))
            {
                //console.log('ERROR: Scrollbar: already initialised.');
            ...
            {
                console.log('ERROR: Scrollbar: overflow | scroll property is missing.');
            ...
            var _initContainer = function(){
                ...
                switch (_vertBar)
                {
                    case 'bar':
                        _arrow_t.hide();
                        _space_t.css('top', '9px');
                        _track_v.css({
                            'height': vsize - _size.skin.space_t_h - _size.skin.space_b_h + 'px',
                            'top': _size.skin.space_t_h + 'px'
                        });
                        _space_b.css('top', _size.live.bar_v_h - _size.skin.space_b_h - 9 + 'px');
                        _arrow_b.hide();
                        break;
                    case 'updownbar':
                        _arrow_t.css('top', '0px');
                        _arrow_b.css('top', _size.skin.arrow_t_h + 'px');
                        _space_t.css('top', _size.skin.arrow_t_h + _size.skin.arrow_b_h + 9 + 'px');
                        _track_v.css({
                    ...
                    case 'barupdown':
                        ...
                        _space_b.css('top', _size.live.bar_v_h - _size.skin.arrow_t_h - _size.skin.arrow_b_h - _size.skin.space_b_h - 9 + 'px');
                    ...
                    default:
                        _arrow_t.css('top', '0px');
                        _space_t.css('top', _size.skin.arrow_t_h + 9 + 'px');
                        _track_v.css({
                            'height': vsize - _size.skin.arrow_t_h - _size.skin.space_t_h - _size.skin.space_b_h - _size.skin.arrow_b_h + 'px',
                            'top': _size.skin.arrow_t_h + _size.skin.space_t_h + 'px'
                        });
                        _space_b.css('top', _size.live.bar_v_h - _size.skin.arrow_b_h - _size.skin.space_b_h - 9 + 'px');
                        _arrow_b.css('top', _size.live.bar_v_h - _size.skin.arrow_b_h + 'px');
                }
                // horizontal bar
                switch (_horzBar)
...
/*                
                console.log(_container.get(0).scrollTop);
                console.log(_container.get(0).scrollHeight);
                console.log('#1 scroll - scrollHeight: ' + _size.live.scrollHeight + ' height: ' + (_size.live.height - _size.live.bar_h_h));
*/
                if (_size.live.scrollHeight >= _size.live.height - _size.live.bar_h_h)
                {
                    var pos, timed;
                    ...
                    //console.log('#2 scroll - pos: ' + pos);

                    if (_options.smoothScroll && !_inSwipe)
                    {
                        $(_container).stop(false, true).velocity({
                            'scrollTop':pos
                        }, {
                            duration:timed,
                            easing:'linear',
                    ...
                    {
                       // console.log('#3 scroll - pos: ' + pos);
                        _container.get(0).scrollTop = pos;
                    }
                }
                else
                {
                    // console.log('#4 scroll - zero');
                    _container.get(0).scrollTop = _size.live.scrollHeight + delta;
                    //_container.get(0).scrollTop = 0;
                }
                $this.trigger('scroll');
                _initVerticals();
                ...
                if (typeof data.element != 'undefined')
                {
                    ...
                    if (obj.get(0) && $this.has(obj.get(0)))
                    {
                        ...
                        if (center)
                        {
                            ...
                            {
                                if (_options.scrollTo !== null && typeof(_options.scrollTo) === 'string')
                                {
                                    //console.log('ScrollTo: ' + _options.scrollTo);
                                    switch(_options.scrollTo)
                                    {
                                        case 'top': {
                        ...
                        //console.log('Pos top: ' + pos.top + ' Scroll Top: ' + $(_container).scrollTop());
                        var offsety = (data.offsety ? parseInt(data.offsety) : 0);
                        if (Math.abs($(_container).scrollTop() - Math.abs(pos.top)) >= offsety)
                        {
                            if (data.animate)
                            {
                                //console.log('WTF: ' + pos.top);
                                $(_container).velocity("finish", true).velocity({
                                    'scrollTop': pos.top
                                }, {
